...
Процедура ВКМ_РассчитатьОтпуск()
	...
	Пока Результат.Следующий() Цикл
		...
		Если Результат.ОтработаноЧасовБаза = 0 Или Результат.ПериодДействияНачало = Null
			Или Результат.ПериодДействияКонец = Null Тогда

			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Для строки " + Результат.НомерСтроки
				+ " отсутствует база для расчета отпуска или отсутствуют данные об отработанных часах. Расчет не выполнен.";
			Сообщение.Сообщить();
			Продолжить;
